  ...
  <div v-if="dataAward">
    ...
    <div class="container-fluid mt-1 mb-1 ">
      ...
      <div class="row mt-3 ">
            ...
            <div class="box-shadow-me rounded bg-white box  nav-bottom">
              <div class="card-header bg-primary">
                ...
                <div class="text-center mb-2">
                  ...
                  <h4 class="header-font text-white"><b>{{dataAward.name}} จำนวนรางวัล {{dataAward.amount - dataAward.lucky.length}} รางวัล</b> </h4>
                  <span class="text-laa text-white">จำนวนผู้มีสิทธ์ลุ้นรับรางวัล {{oldUser.length}} คน</span>
                </div>
              ...
              <div class="card-footer">
                <button v-if="!success" :disabled="statusPlay  || dataAward.status" class="btn btn-dark btn-block btn-lg mt-3 shadow-lg" @click="startRandom(dataAward)"> เริ่ม</button>
        ...
        <div class="col-xl-6 pt-3" id="print">
          ...
          <div class="box-shadow-me rounded bg-white  " v-if="listUser.length != 0">
            ...
            <table class="table table-borderless table-striped  rounded ">
              ...
              <tbody>
                <tr style="font-size:20px" v-for="(u,index) in listUser" :key="index" :class="'testx '+u.PERSONID">
                  <td>
                   
                      <img src="./../assets/award.png" width="30px" v-if="u.isSave">
                     {{index + 1}}.</td>
                  <td>{{u.PERSONID}}</td>
                  ...
                  <td >
                    <i style="cursor: pointer;" v-if="!u.isSave" class="text-danger fa fa-times" @click="removeUser(index)"  aria-hidden="true"></i>
                  </td>
                </tr>
    ...
    if (data) {
      if (data.filter((d) => d.id == id)[0]) {
        ...
        this.title = [
          ...
          "รางวัล " + this.dataAward.name,
          "จำนวน " + this.dataAward.amount,
        ];
  ...
  methods: {
    ...
    saveComplete() {
      this.$swal({
        ...
        text:
          "หลังจากทำการบันทึกข้อมูลแล้วระบบจะทำการหักสิทธ์ของผู้เข้าร่วม และ ระบบจะสร้างรายงานอัตโนมัติ",
        type: "warning",
        showCancelButton: true,
        confirmButtonColor: "#3085d6",
        cancelButtonColor: "#d33",
        ...
        if (result.value) {
          ...
          this.success = false;
          this.disPrivilege();
          if (this.dataAward.amount == this.dataAward.lucky.length) {
      ...
      if (data) {
        ...
        this.listUser.forEach((lu) => {
          ...
          if (key != -1 && !lu.isSave) {
            ...
            listUser[key].inventory.push({
              id: this.dataAward.id,
              name: this.dataAward.name,
            });
            lu.isSave = true;
        ...
        if (data) {
          ...
          if (key != -1) {
            ...
            if (this.listUser.length == this.dataAward.amount) {
    ...
    startRandom(gift) {
      const amount = gift.amount - gift.lucky.length;
      if (amount != 0) {
        this.success = false;
        ...
        this.listUserTemp = JSON.parse(JSON.stringify(this.oldUser));
        // this.listUser = [];
        for (let index = 1; index <= amount; index++) {
          var indexRandom = Math.floor(
            Math.random() * this.listUserTemp.length
          );
